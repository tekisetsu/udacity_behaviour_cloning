...
def normalize(img):
    """
    normalizes an image, i took this step out of preprocessing because after this step we no longer have
    an image that we can visualize. 
    :param img: np.array
...
def telemetry(sid, data):
    if data:
        ...
        # add preprocessing
        input_image = normalize(image_preprocessing(image_array))
        ...
        steering_angle = float(model.predict(np.array(input_image), batch_size=1))
